//Taken from https://github.com/canonizer/mandelbrot-dyn/blob/master/mandelbrot-dyn/mandelbrot-dyn.cu
...
#define cucheck(call) \
	...
	fprintf(stderr, "%s (%d): %s (%d) in %s", __FILE__, __LINE__, err_str, res, #call);	\
	exit(-1);\
		}\
		}
